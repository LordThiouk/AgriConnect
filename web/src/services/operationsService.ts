...
import { Operation, OperationFilters, PaginationParams, PaginatedResponse } from '../types';
...
export class OperationsService {
  static async getOperations(
    ...
    try {
      ...
      const { data: countData, error: countError } = await supabase.rpc('count_operations_for_filters', {
        plot_uuid: filters.plot_id || null,
        crop_uuid: filters.crop_id || null,
        operation_type_filter: filters.operation_type || null,
        date_from: filters.date_from ? filters.date_from.toString() : null,
      ...
      const operations: Operation[] = (data || []).map((op: any) => ({
        ...
        performer_name: op.performer_name,
        performer_phone: op.performer_phone,
        notes: op.notes,
  ...
  static async getOperationsByPlot(plotId: string): Promise<Operation[]> {
    try {
      const response = await this.getOperations({ plot_id: plotId }, { page: 1, limit: 100 });
  ...
  static async getOperationsByCrop(cropId: string): Promise<Operation[]> {
    try {
      const response = await this.getOperations({ crop_id: cropId }, { page: 1, limit: 100 });
      return response.data;
    } catch (error) {
  ...
  static async getOperationById(id: string): Promise<Operation | null> {
    try {
      ...
      return {
        ...
        performed_by: operation.performer_id,
        performer_name: operation.performer_name,
        performer_phone: operation.performer_phone,
