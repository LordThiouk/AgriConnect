-- Fix get_plots_with_geolocation to use correct cooperative column name
-- The profiles table uses 'cooperative', not 'cooperative_id'

-- Drop existing function
DROP FUNCTION IF EXISTS get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) CASCADE;

-- Updated function with correct cooperative column
...
BEGIN
  ...
  SELECT 
    ...
    pr.cooperative as producer_cooperative_id  -- Use 'cooperative' column from profiles
  FROM plots p
  ...
  WHERE 
    (search_param IS NULL OR 
     ...
     pr.display_name ILIKE '%' || search_param || '%')  -- Use display_name for search
    ...
    AND (cooperative_id_param IS NULL OR pr.cooperative = cooperative_id_param)  -- Use 'cooperative' column
...
-- Add comment
COMMENT ON FUNCTION get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) IS 'Récupère les parcelles avec géolocalisation, farm_file_plot_id et colonnes profiles correctes (cooperative)';
