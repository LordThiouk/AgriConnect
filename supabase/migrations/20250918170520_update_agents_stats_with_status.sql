-- Update get_agents_stats to use the status column properly
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM public.visits WHERE status = 'completed') as total_visits, -- Now using status column
    COALESCE(
