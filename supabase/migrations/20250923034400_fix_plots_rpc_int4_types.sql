-- Fix get_plots_with_geolocation with correct INTEGER types (int4)
-- soil_ph, slope_percent, elevation_meters are INTEGER in database, not NUMERIC
...
RETURNS TABLE (
  ...
  soil_type TEXT,
  soil_ph INTEGER,                  -- INTEGER (int4) - can be NULL
  water_source TEXT,
  irrigation_type TEXT,
  slope_percent INTEGER,            -- INTEGER (int4) - can be NULL  
  elevation_meters INTEGER,         -- INTEGER (int4) - can be NULL
  status TEXT,
  notes TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    ffp.area_hectares,
    ffp.soil_type,
    ffp.soil_ph,                    -- INTEGER (int4)
    ffp.water_source,
    ffp.irrigation_type,
    ffp.slope_percent,              -- INTEGER (int4)
    ffp.elevation_meters,           -- INTEGER (int4)
    ffp.status,
    ffp.notes,
    p.created_at,
    p.updated_at,
...
-- Add comment
COMMENT ON FUNCTION get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) IS 'Récupère les parcelles avec géolocalisation et farm_file_plot_id (types INTEGER corrects)';
