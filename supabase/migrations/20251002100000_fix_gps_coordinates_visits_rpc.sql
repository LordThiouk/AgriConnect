-- FIX: Corriger l'extraction des coordonnées GPS dans get_agent_all_visits_with_filters
-- ============================================================================
-- Objectif: Corriger l'erreur "operator does not exist: geometry ? unknown"
-- Solution: Utiliser ST_AsGeoJSON pour convertir geometry en jsonb avant extraction
-- ============================================================================

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS get_agent_all_visits_with_filters(uuid, text) CASCADE;

-- Recréer la fonction avec extraction GPS corrigée
...
BEGIN
    ...
    SELECT 
        ...
        -- Coordonnées GPS extraites de center_point (corrigé)
        CASE 
            WHEN pl.center_point IS NOT NULL THEN
                (ST_AsGeoJSON(pl.center_point)::jsonb->'coordinates'->>1)::numeric -- latitude (index 1)
            ELSE NULL
        END as lat,
        CASE 
            WHEN pl.center_point IS NOT NULL THEN
                (ST_AsGeoJSON(pl.center_point)::jsonb->'coordinates'->>0)::numeric -- longitude (index 0)
            ELSE NULL
        END as lon
...
COMMENT ON FUNCTION get_agent_all_visits_with_filters(uuid, text) IS 
  'Récupère les visites d''un agent avec filtres de date et statut - Version avec coordonnées GPS corrigée';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '╔════════════════════════════════════════════════════════╗';
  RAISE NOTICE '║  RPC get_agent_all_visits_with_filters GPS CORRIGÉE   ║';
  RAISE NOTICE '║  Correction: ST_AsGeoJSON pour extraction coordonnées ║';
  RAISE NOTICE '╚════════════════════════════════════════════════════════╝';
  RAISE NOTICE '';
  RAISE NOTICE 'Correction appliquée:';
  RAISE NOTICE '  ✓ ST_AsGeoJSON(pl.center_point)::jsonb pour conversion geometry->jsonb';
  RAISE NOTICE '  ✓ Extraction lat/lon depuis les coordonnées GeoJSON';
  RAISE NOTICE '';
  RAISE NOTICE 'Résultat: Visites avec coordonnées GPS fonctionnelles';
  RAISE NOTICE '';
  RAISE NOTICE '════════════════════════════════════════════════════════';
END $$;
