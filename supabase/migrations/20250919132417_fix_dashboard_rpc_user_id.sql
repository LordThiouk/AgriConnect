-- Fix dashboard RPC functions to accept user_id instead of profiles.id
-- The functions should convert user_id to profiles.id internally

-- Drop existing functions first
DROP FUNCTION IF EXISTS get_agent_dashboard_stats(uuid);
DROP FUNCTION IF EXISTS get_agent_today_visits(uuid);
DROP FUNCTION IF EXISTS get_agent_terrain_alerts(uuid);

-- Create get_agent_dashboard_stats to accept user_id
...
BEGIN
    ...
    -- 4. Count all plots for these producers via farm_file_plots
...
-- Update get_agent_today_visits to accept user_id
...
-- Update get_agent_terrain_alerts to accept user_id
