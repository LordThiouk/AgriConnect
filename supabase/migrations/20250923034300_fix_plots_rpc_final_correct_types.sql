-- Fix get_plots_with_geolocation with correct types based on actual database schema
-- soil_ph, slope_percent, elevation_meters can be NULL, so use NUMERIC to handle both integer and decimal values
...
RETURNS TABLE (
  ...
  producer_id UUID,
  name TEXT,
  area_hectares NUMERIC,            -- This is a number (2.5 in sample)
  soil_type TEXT,
  soil_ph NUMERIC,                  -- Can be NULL, use NUMERIC to handle decimals
  water_source TEXT,
  irrigation_type TEXT,
  slope_percent NUMERIC,            -- Can be NULL, use NUMERIC to handle decimals
  elevation_meters NUMERIC,         -- Can be NULL, use NUMERIC to handle decimals
  status TEXT,
  notes TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    ffp.area_hectares,
    ffp.soil_type,
    ffp.soil_ph,                    -- NUMERIC (can be NULL)
    ffp.water_source,
    ffp.irrigation_type,
    ffp.slope_percent,              -- NUMERIC (can be NULL)
    ffp.elevation_meters,           -- NUMERIC (can be NULL)
    ffp.status,
    ffp.notes,
    p.created_at,
    p.updated_at,
...
-- Add comment
COMMENT ON FUNCTION get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) IS 'Récupère les parcelles avec géolocalisation et farm_file_plot_id (types corrects basés sur le schéma réel)';
