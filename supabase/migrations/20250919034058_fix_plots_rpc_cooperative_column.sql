-- Fix the plots RPC functions to use correct cooperative column

-- Drop and recreate get_plots_with_geolocation function
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN ffp.center_point IS NOT NULL THEN ST_Y(ffp.center_point)
      WHEN ffp.geom IS NOT NULL THEN ST_Y(ST_Centroid(ffp.geom))
      ELSE NULL
    END as latitude,
    CASE 
      WHEN ffp.center_point IS NOT NULL THEN ST_X(ffp.center_point)
...
-- Drop and recreate get_plots_with_geolocation_count function
