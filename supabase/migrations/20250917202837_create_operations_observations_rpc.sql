-- RPC Functions for Operations and Observations
-- These functions handle the complex relationships with crops and plots

-- Function to get operations with crop and plot information
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(p.name, 'Parcelle inconnue') as plot_name
  FROM operations o
  LEFT JOIN crops c ON o.crop_id = c.id
  LEFT JOIN plots p ON o.plot_id = p.id
  WHERE 
    (producer_uuid IS NULL OR c.plot_id IN (
      SELECT fp.plot_id 
...
-- Function to get observations with crop and plot information
CREATE OR REPLACE FUNCTION get_observations_with_details(
  producer_uuid uuid DEFAULT NULL,
  limit_count integer DEFAULT 50,
  offset_count integer DEFAULT 0,
  search_term text DEFAULT NULL,
...
BEGIN
  ...
  LEFT JOIN plots p ON obs.plot_id = p.id
  WHERE 
    (producer_uuid IS NULL OR c.plot_id IN (
      SELECT fp.plot_id 
      FROM farm_file_plots fp
...
-- Function to count operations for a producer
...
-- Function to count observations for a producer
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION get_operations_with_details TO authenticated;
GRANT EXECUTE ON FUNCTION get_observations_with_details TO authenticated;
GRANT EXECUTE ON FUNCTION count_operations_for_producer TO authenticated;
GRANT EXECUTE ON FUNCTION count_observations_for_producer TO authenticated;
