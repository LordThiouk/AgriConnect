-- Fix get_plots_with_geolocation with correct types from database schema
-- Based on actual database types: slope_percent, elevation_meters, soil_ph are INTEGER, not NUMERIC
...
RETURNS TABLE (
  ...
  soil_type TEXT,
  soil_ph INTEGER,                  -- Corrected: INTEGER not NUMERIC
  water_source TEXT,
  irrigation_type TEXT,
  slope_percent INTEGER,            -- Corrected: INTEGER not NUMERIC
  elevation_meters INTEGER,         -- Corrected: INTEGER not NUMERIC
  status TEXT,
  notes TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    ffp.area_hectares,
    ffp.soil_type,
    ffp.soil_ph,                    -- INTEGER type
    ffp.water_source,
    ffp.irrigation_type,
    ffp.slope_percent,              -- INTEGER type
    ffp.elevation_meters,           -- INTEGER type
    ffp.status,
    ffp.notes,
    p.created_at,
    p.updated_at,
...
-- Add comment
COMMENT ON FUNCTION get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) IS 'Récupère les parcelles avec géolocalisation et farm_file_plot_id (types corrigés)';
