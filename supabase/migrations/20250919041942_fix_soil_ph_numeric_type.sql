-- Fix soil_ph return type to NUMERIC to match database column type numeric(3,1)

-- Drop and recreate get_plots_with_geolocation function with correct NUMERIC type for soil_ph
...
RETURNS TABLE (
  ...
  soil_type TEXT,
  soil_ph NUMERIC,  -- Return NUMERIC to match database column type numeric(3,1)
  water_source TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    ffp.area_hectares,
    ffp.soil_type,
    ffp.soil_ph::NUMERIC,  -- Cast to NUMERIC to match return type
    ffp.water_source,
    ffp.irrigation_type,
    ffp.slope_percent,
