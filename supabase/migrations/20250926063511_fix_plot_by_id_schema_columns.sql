-- Migration: Correction finale get_plot_by_id - utilisation des bonnes colonnes
...
BEGIN
  ...
  SELECT 
    ...
    -- Agrégations avec les BONNES colonnes
    (
      SELECT COUNT(*) 
      FROM crops c 
      WHERE c.farm_file_plot_id = ffp.id OR c.plot_id = ffp.plot_id
    ) as crops_count,
    (
      SELECT op.operation_type 
      FROM operations op 
      WHERE op.plot_id = ffp.plot_id 
