-- Create RPC functions for operations with plot and crop filtering
-- This allows the frontend to fetch operations by plot_id or crop_id with details
...
-- Add comments
COMMENT ON FUNCTION get_operations_with_details(uuid, uuid, text, date, date, integer, integer) IS 'Récupère les opérations agricoles avec détails par parcelle, culture ou type';
COMMENT ON FUNCTION count_operations_for_filters(uuid, uuid, text, date, date) IS 'Compte les opérations agricoles selon les filtres';
