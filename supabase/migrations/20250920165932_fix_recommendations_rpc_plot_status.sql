-- Fix get_recommendations_with_details RPC to remove non-existent columns
-- The plots table doesn't have status column, and we need to check what columns actually exist
...
RETURNS TABLE (
  ...
  -- Plot details (only existing columns)
  plot_name TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    -- Plot details (only existing columns from plots table)
  ...
  LEFT JOIN producers p ON r.producer_id = p.id
  LEFT JOIN plots pl ON r.plot_id = pl.id
  LEFT JOIN farm_file_plots ffp ON pl.id = ffp.plot_id  -- Join farm_file_plots for area_hectares, soil_type, water_source
  LEFT JOIN crops c ON r.crop_id = c.id
  LEFT JOIN cooperatives co ON p.cooperative_id = co.id
  WHERE 
    (filters->>'search' IS NULL OR 
