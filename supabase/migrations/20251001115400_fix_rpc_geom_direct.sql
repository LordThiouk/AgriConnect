-- ============================================================================
-- FIX FINAL SIMPLIFIÉ: Retourner geom directement (déjà en JSONB)
-- ============================================================================
-- Erreur: "CASE/WHEN could not convert type jsonb to geometry"
-- Cause: La conversion conditionnelle essayait de traiter JSONB comme GEOMETRY
-- Solution: Retourner p.geom directement (l'analyse confirme qu'il est en JSONB)
-- ============================================================================
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(ST_Y(p.center_point::geometry), 0)::double precision AS latitude,
    COALESCE(ST_X(p.center_point::geometry), 0)::double precision AS longitude,
    -- geom: Retourné directement (déjà en JSONB selon analyse)
    p.geom,
    -- center_point: Converti depuis GEOMETRY
...
COMMENT ON FUNCTION public.get_plots_with_geolocation IS 
  'Récupère les parcelles avec géolocalisation. geom retourné directement (JSONB), center_point converti.';
...
BEGIN
  ...
  RAISE NOTICE '╚════════════════════════════════════════════════════════╝';
  RAISE NOTICE '';
  RAISE NOTICE 'Solution simplifiée:';
  RAISE NOTICE '  ✓ geom: Retourné directement (p.geom)';
  RAISE NOTICE '  ✓ center_point: ST_AsGeoJSON()::jsonb';
  RAISE NOTICE '  ✓ lat/lon: COALESCE pour NULL-safety';
  RAISE NOTICE '';
  RAISE NOTICE 'L''erreur CASE/WHEN devrait être résolue ✓';
  RAISE NOTICE '';
  RAISE NOTICE '════════════════════════════════════════════════════════';
END $$;
