-- ============================================================================
-- FIX: get_agent_dashboard_unified - Correction GROUP BY pour alertes
-- ============================================================================
-- Problème: Erreur "column rec.created_at must appear in the GROUP BY clause"
-- Solution: Utiliser une sous-requête pour les alertes comme pour les visites
-- ============================================================================

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS get_agent_dashboard_unified(uuid, text) CASCADE;

-- Recréer la fonction avec correction GROUP BY pour alertes
...
BEGIN
    ...
    -- 5. Récupérer les alertes terrain (correction GROUP BY avec sous-requête)
    SELECT COALESCE(json_agg(
        json_build_object(
            'id', alert_data.id,
            ...
            'producer_name', alert_data.producer_name,
            'plot_name', alert_data.plot_name
        )
    ), '[]'::json) INTO v_alerts
    FROM (
        SELECT 
            rec.id,
            COALESCE(rec.title, 'Alerte terrain') as title,
            COALESCE(rec.message, 'Aucune description') as description,
            COALESCE(rec.priority, 'medium') as severity,
            rec.created_at,
...
COMMENT ON FUNCTION get_agent_dashboard_unified(uuid, text) IS 
  'RPC unifié pour récupérer toutes les données du dashboard agent en un seul appel (stats, visites, alertes) - Version avec GROUP BY corrigé pour alertes';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '╔════════════════════════════════════════════════════════╗';
  RAISE NOTICE '║  RPC get_agent_dashboard_unified GROUP BY CORRIGÉ     ║';
  RAISE NOTICE '║  Correction: Sous-requête pour alertes                ║';
