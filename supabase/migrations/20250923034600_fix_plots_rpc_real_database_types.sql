-- Fix get_plots_with_geolocation with real database types from farm_file_plots
-- farm_file_plots was renamed from plots, so it has the original types:
-- soil_ph: DECIMAL(3,1), slope_percent: INTEGER, elevation_meters: INTEGER, area_hectares: DECIMAL(8,2)
...
RETURNS TABLE (
  ...
  producer_id UUID,
  name TEXT,
  area_hectares DECIMAL(8,2),       -- DECIMAL(8,2) from original plots table
  soil_type TEXT,
  soil_ph DECIMAL(3,1),             -- DECIMAL(3,1) from original plots table
  water_source TEXT,
  irrigation_type TEXT,
  slope_percent INTEGER,            -- INTEGER from original plots table
  elevation_meters INTEGER,         -- INTEGER from original plots table
  status TEXT,
  notes TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    p.producer_id,
    p.name,
    ffp.area_hectares,              -- DECIMAL(8,2)
    ffp.soil_type,
    ffp.soil_ph,                    -- DECIMAL(3,1)
    ffp.water_source,
    ffp.irrigation_type,
    ffp.slope_percent,              -- INTEGER
    ffp.elevation_meters,           -- INTEGER
    ffp.status,
    ffp.notes,
    p.created_at,
    p.updated_at,
...
-- Add comment
COMMENT ON FUNCTION get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) IS 'Récupère les parcelles avec géolocalisation et farm_file_plot_id (types réels de la base de données)';
