-- Fix function overloading conflict for get_operations_with_details
-- Drop all existing versions and recreate with single signature

-- Drop all existing versions of the function
DROP FUNCTION IF EXISTS get_operations_with_details(uuid, uuid, text, text, text, integer, integer) CASCADE;
DROP FUNCTION IF EXISTS get_operations_with_details(uuid, uuid, text, date, date, integer, integer) CASCADE;

-- Function to get operations with crop, plot and AGENT information (SINGLE SIGNATURE)
...
-- Add comment
COMMENT ON FUNCTION get_operations_with_details(uuid, uuid, text, text, text, integer, integer) IS 'Récupère les opérations avec détails des cultures, parcelles et informations agent (SINGLE SIGNATURE)';
