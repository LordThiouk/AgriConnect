...
RETURNS TABLE (
  ...
  slope_percent NUMERIC,             -- farm_file_plots.slope_percent (number | null in TS)
  elevation_meters NUMERIC,          -- farm_file_plots.elevation_meters (number | null in TS)
...
BEGIN
  ...
  SELECT 
    ...
    ffp.slope_percent,              -- NUMERIC from farm_file_plots (can be NULL)
    ffp.elevation_meters,           -- NUMERIC from farm_file_plots (can be NULL)
...
-- Add comment
COMMENT ON FUNCTION get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) IS 'Récupère les parcelles avec géolocalisation et farm_file_plot_id (types correspondant exactement aux types TypeScript)';
