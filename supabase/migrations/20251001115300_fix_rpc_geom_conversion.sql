-- ============================================================================
-- FIX FINAL: Conversion GEOMETRY → JSONB dans get_plots_with_geolocation
-- ============================================================================
-- Problème: geom est stocké en GEOMETRY(Polygon,4326) dans la base
--           mais la fonction RPC déclare JSONB
...
BEGIN
  ...
  SELECT
    ...
    p.created_at,
    p.updated_at,
    -- Latitude et longitude depuis center_point
    CASE 
      WHEN p.center_point IS NOT NULL THEN ST_Y(p.center_point::geometry)::double precision
      ELSE NULL
    END AS latitude,
    CASE 
      WHEN p.center_point IS NOT NULL THEN ST_X(p.center_point::geometry)::double precision
      ELSE NULL
    END AS longitude,
    -- Convertir geom GEOMETRY → JSONB
    CASE 
      WHEN p.geom IS NOT NULL THEN 
        CASE 
          WHEN pg_typeof(p.geom)::text LIKE '%geometry%' THEN ST_AsGeoJSON(p.geom::geometry)::jsonb
          ELSE p.geom  -- Déjà en JSONB
        END
      ELSE NULL
    END AS geom,
    -- Convertir center_point GEOMETRY → JSONB
    CASE 
      WHEN p.center_point IS NOT NULL THEN ST_AsGeoJSON(p.center_point::geometry)::jsonb
      ELSE NULL
    END AS center_point,
...
COMMENT ON FUNCTION public.get_plots_with_geolocation IS 
  'Récupère les parcelles avec géolocalisation. Conversion GEOMETRY → JSONB pour compatibilité frontend.';
...
BEGIN
  ...
  RAISE NOTICE '  ✓ geom: Conversion conditionnelle GEOMETRY → JSONB';
  RAISE NOTICE '  ✓ center_point: Conversion ST_AsGeoJSON()::jsonb';
  RAISE NOTICE '  ✓ latitude/longitude: Protection contre NULL';
  RAISE NOTICE '';
  RAISE NOTICE 'L''erreur "geometry does not match jsonb" devrait être résolue ✓';
  RAISE NOTICE '';
  RAISE NOTICE '════════════════════════════════════════════════════════';
END $$;
