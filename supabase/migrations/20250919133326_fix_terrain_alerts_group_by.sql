-- Fix terrain alerts RPC function to resolve GROUP BY clause error
-- The issue is that we're using o.severity in ORDER BY but not in GROUP BY

-- Drop and recreate the function with proper GROUP BY handling
