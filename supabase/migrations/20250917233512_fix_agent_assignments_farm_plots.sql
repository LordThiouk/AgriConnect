-- Fix agent assignments and use farm_file_plots table
-- The agent Auth ID is b00a283f-0a46-41d2-af95-8a256c9c2771
...
BEGIN
  ...
  SELECT COUNT(*) INTO producer_count FROM producers;
  SELECT COUNT(*) INTO farm_file_plot_count FROM farm_file_plots;
  
  RAISE NOTICE 'Agents: %, Assignments: %, Producers: %, Farm File Plots: %', 
    agent_count, assignment_count, producer_count, farm_file_plot_count;
    
  -- Show the specific agent Auth ID
  IF EXISTS (SELECT 1 FROM profiles WHERE user_id = 'b00a283f-0a46-41d2-af95-8a256c9c2771'::uuid) THEN
    RAISE NOTICE 'Agent with Auth ID b00a283f-0a46-41d2-af95-8a256c9c2771 EXISTS';
  ELSE
    RAISE NOTICE 'Agent with Auth ID b00a283f-0a46-41d2-af95-8a256c9c2771 NOT FOUND';
  END IF;
END $$;
...
-- Create farm_file_plots for these producers with GPS coordinates
-- First, we need to create plots in the plots table, then link them to farm_file_plots
...
-- Now create farm_file_plots linked to the plots we just created
