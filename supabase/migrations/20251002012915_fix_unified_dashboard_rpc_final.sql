-- ============================================================================
-- FIX: get_agent_dashboard_unified - Correction finale GROUP BY
-- ============================================================================
-- Problème: Erreur "column v.visit_date must appear in the GROUP BY clause"
-- Solution: Utiliser array_agg avec ORDER BY au lieu de json_agg
-- ============================================================================

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS get_agent_dashboard_unified(uuid, text) CASCADE;

-- Recréer la fonction avec correction GROUP BY
...
COMMENT ON FUNCTION get_agent_dashboard_unified(uuid, text) IS 
  'RPC unifié pour récupérer toutes les données du dashboard agent en un seul appel (stats, visites, alertes) - Version finale corrigée';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '╔════════════════════════════════════════════════════════╗';
  RAISE NOTICE '║  RPC get_agent_dashboard_unified FINALE               ║';
  RAISE NOTICE '║  Correction: Sous-requête pour éviter GROUP BY        ║';
