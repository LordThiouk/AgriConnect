-- Migration: Correction des types INTEGER pour get_plots_with_geolocation
-- Objectif: Aligner elevation_meters et slope_percent avec les types réels de la base (INTEGER)
...
RETURNS TABLE (
  ...
  slope_percent INTEGER,        -- Corrigé: INTEGER (pas NUMERIC)
  elevation_meters INTEGER,      -- Corrigé: INTEGER (pas NUMERIC)
  status TEXT,
  notes TEXT,
...
-- Comment
COMMENT ON FUNCTION get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) IS 'Récupère les parcelles avec géolocalisation (types INTEGER pour slope_percent et elevation_meters)';
