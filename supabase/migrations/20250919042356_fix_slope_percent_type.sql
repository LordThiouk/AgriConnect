-- Fix slope_percent return type to INTEGER to match database column type

-- Drop and recreate get_plots_with_geolocation function with correct INTEGER type for slope_percent
...
RETURNS TABLE (
  ...
  slope_percent INTEGER,  -- Changed from NUMERIC to INTEGER to match database column
...
BEGIN
  ...
  SELECT 
    ...
    ffp.water_source,
    ffp.irrigation_type,
    ffp.slope_percent::INTEGER,  -- Cast to INTEGER to match return type
