-- ============================================================================
-- FIX: get_agent_all_visits_with_filters - Correction agent_id dans WHERE
-- ============================================================================
-- Problème: Le RPC utilise v.agent_id = p_user_id au lieu de v.agent_id = v_agent_profile_id
-- Solution: Utiliser v_agent_profile_id dans la clause WHERE
-- ============================================================================
...
BEGIN
    ...
    -- 3. Return visits with filters (utiliser v_agent_profile_id au lieu de p_user_id)
    RETURN QUERY
    SELECT 
        v.id,
    ...
    WHERE v.agent_id = v_agent_profile_id  -- CORRECTION: utiliser v_agent_profile_id
...
COMMENT ON FUNCTION get_agent_all_visits_with_filters(uuid, text) IS 
  'Récupère les visites d''un agent avec filtres de date - Version corrigée avec agent_id';
...
BEGIN
  ...
  RAISE NOTICE '║  Correction: v.agent_id = v_agent_profile_id           ║';
  RAISE NOTICE '╚════════════════════════════════════════════════════════╝';
  RAISE NOTICE '';
END $$;
