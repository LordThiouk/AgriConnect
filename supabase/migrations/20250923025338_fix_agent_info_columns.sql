-- Fix agent info columns in observations and operations RPC functions
-- The profiles table uses display_name instead of first_name/last_name
...
-- Function to get observations with crop, plot and AGENT information (V3 - FIXED COLUMNS)
...
BEGIN
  ...
  SELECT 
    ...
    obs.recommendations,
    obs.observed_by,
    COALESCE(p.display_name, 'Agent inconnu') as agent_name,  -- FIXED: use display_name
...
-- Function to get operations with crop, plot and AGENT information (V3 - FIXED COLUMNS)
...
BEGIN
  ...
  SELECT 
    ...
    o.total_cost,
    o.performer_id,
    COALESCE(p.display_name, 'Agent inconnu') as performer_name,  -- FIXED: use display_name
...
-- Function to count observations for a producer (V3 - KEEP EXISTING)
...
-- Function to count operations for a producer (V3 - KEEP EXISTING)
...
-- Add comments
COMMENT ON FUNCTION get_observations_with_details_v3 IS 'Récupère les observations avec détails des cultures, parcelles et informations agent (V3 - COLONNES CORRIGÉES)';
COMMENT ON FUNCTION get_operations_with_details_v3 IS 'Récupère les opérations avec détails des cultures, parcelles et informations agent (V3 - COLONNES CORRIGÉES)';
