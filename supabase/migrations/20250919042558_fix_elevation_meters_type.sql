-- Fix elevation_meters return type to INTEGER to match database column type

-- Drop and recreate get_plots_with_geolocation function with correct INTEGER type for elevation_meters
...
RETURNS TABLE (
  ...
  elevation_meters INTEGER,  -- Changed from NUMERIC to INTEGER to match database column
  status TEXT,
  notes TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    ffp.elevation_meters::INTEGER,  -- Cast to INTEGER to match return type
    ffp.status,
    ffp.notes,
    p.created_at,
    p.updated_at,
