-- Fix get_agent_performance function to match frontend AgentPerformance interface
-- Drop existing function
DROP FUNCTION IF EXISTS get_agent_performance(UUID);

-- Create corrected version with all fields expected by frontend
...
BEGIN
  ...
  -- Calculate photos per plot (simplified: based on observations with photos)
  SELECT COALESCE(
    (SELECT (COUNT(*) FILTER (WHERE photo_url IS NOT NULL AND photo_url != ''))::NUMERIC / NULLIF(v_total_plots, 0)
     FROM public.observations obs
     JOIN public.agent_assignments aa ON obs.observed_by = aa.agent_id
...
-- Add comment
COMMENT ON FUNCTION get_agent_performance(UUID) IS 'Returns complete performance metrics for an agent matching frontend AgentPerformance interface';
