-- Corriger le type de date dans la fonction RPC get_crop_by_id
-- La colonne sowing_date est de type DATE, pas TIMESTAMPTZ
...
RETURNS TABLE (
  ...
  crop_type TEXT,
  variety TEXT,
  sowing_date DATE,  -- Changé de TIMESTAMPTZ à DATE
  status TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    c.updated_at
  FROM crops c
  JOIN farm_file_plots ffp ON ffp.id = c.plot_id
