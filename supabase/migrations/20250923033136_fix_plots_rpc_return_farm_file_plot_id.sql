-- Fix get_plots_with_geolocation to return farm_file_plot_id and improve geolocation
-- This will allow PlotDetailsModal to use the correct ID for fetching crops and operations

-- Drop existing function
DROP FUNCTION IF EXISTS get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) CASCADE;

-- Updated function to return farm_file_plot_id and improved geolocation
...
BEGIN
  ...
  WHERE 
    (search_param IS NULL OR 
     ...
     pr.first_name ILIKE '%' || search_param || '%' OR
     pr.last_name ILIKE '%' || search_param || '%')
...
-- Add comment
COMMENT ON FUNCTION get_plots_with_geolocation(TEXT, UUID, TEXT, TEXT, TEXT, TEXT, UUID, INTEGER, INTEGER) IS 'Récupère les parcelles avec géolocalisation et retourne farm_file_plot_id pour cultures/opérations';
